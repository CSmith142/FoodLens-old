...
class HistoryViewModel: ObservableObject {
    @Published var scannedProducts: [ScannedProduct] = []
    @Published var favorites: [ScannedProduct] = []
    
    func addScannedProduct(_ product: ScannedProduct) {
        scannedProducts.append(product)
    }
    
    func addScannedBarcode(_ barcode: String) {
        let scannedProduct = ScannedProduct(id: UUID(), name: "Manually Entered", brand: "Unknown", userRating: nil, calories: nil, protein: nil, sugar: nil, saturatedFat: nil, fiber: nil)
        scannedProducts.append(scannedProduct)
    }
    
    func removeScannedProduct(at index: Int) {
        scannedProducts.remove(at: index)
    }
    
    func addToFavorites(_ product: ScannedProduct) {
    ...
    func rateProduct(_ product: ScannedProduct) -> Int {
        let ratingAlgorithm = RatingAlgorithm.shared
        let rating = ratingAlgorithm.calculateRating(for: product)
        return rating
    }
    
    func setUserRating(for product: ScannedProduct, rating: Int) {
